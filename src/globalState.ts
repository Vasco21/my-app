  ...
  switch(type){
    ...
    case "SEARCHING":
      state.filteredItems = state.items.filter(item => {
        ...
        return item.itemName.toLowerCase().includes(searching.toLowerCase())
      })
      return{
        ...state,
    ...
    case "FILTER":
      state.filteredItems = payload === "All items" ? state.items : state.items.filter(item => item.promoterName === payload)
      return{ ...state, filterAt: payload as string }

    case "ADD_INITIAL_ITEMS":
      (payload as ItemInterface[]).forEach((product: ItemInterface) => {
        if(!state.categories.includes(product.promoterName)){
          state.categories.push(product.promoterName)
        }
      })
